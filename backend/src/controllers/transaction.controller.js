...
// CREATE par agent (assignée directement + validée d'office)
const createAgentTransaction = async (req, res) => {
  try {
    const result = await service.createAgentTransactionService(
      req.user.id,
      req.body
    );
...
const getTransactionByIdController = async (req, res) => {
  try {
    ...
    res.json({ success: true, data: { transaction: transactionDetails, history } });
  } catch (err) {
...
const finalizeTransaction = async (req, res) => {
  try {
    const result = await service.finalizeTransactionService(req.params.id, {
      id: req.user.id,
      type: req.user.role,
    });
...
const cancelTransaction = async (req, res) => {
  try {
    ...
    res.json({ success: true, data: result, message: "Transaction annulée avec succès" });
  } catch (err) {
    res.status(400).json({ success: false, message: err.message });
  }
};
...
const getSemiAdminShare = async (req, res) => {
  try {
    const percentage = await service.getSemiAdminShare(req.user.id);
    res.json({
      success: true,
      data: {
        percentage,
